...
const FindMe = () => {
    ...
    return <div style={{ zIndex: 10 }} className="cursor-pointer border-2 border-gray-400 rounded bg-white w-[30px] h-[30px] p-1 absolute bottom-[100px] right-3" aria-label="Zoom out" title="Find me"><img onClick={locate} src="../findme.png" alt="Find me" /></div>
}
...
export default ({ data, setData, isDarkMode, prev }) => {
    const [address, setAddress] = React.useState(prev.address);
    ...
    const fetchAddress = () => {
        ...
        instance.get(`address/list/${address}`)
            .then((res) => {
                if (res.status === 200)
                    setApiData(res.data)
                else toast.error(translations.errorOccured);
            })
