...
export default () => {
    ...
    useEffect(() => {
        instance.get('reservations/owned/hotel')
            .then(res => {
    ...
    if (!data && !err)
        ...
        return (
            <>
                ...
                <Accordion variant="splitted" selectionBehavior="toggle" defaultExpandedKeys={["0"]}>
                    {data.map((item, index) => (
                        <AccordionItem key={index} aria-label={"Nr: " + item.reservationNumber} title={"Nr: " + item.reservationNumber}>
                            <Divider className="-mt-4 mb-2" />
                            <p className="dark:text-white text-black">{translations.hotelName}: {item.hotelName}</p>
                            <p className="dark:text-white text-black">{String(translations.room).charAt(0).toUpperCase() + String(translations.room).slice(1)}: {item.name}</p>
                            <p className="dark:text-white text-black">{translations.date}: {item.startDate} - {item.endDate}</p>
