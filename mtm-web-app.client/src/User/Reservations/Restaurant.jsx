...
export default () => {
    ...
    const fetchReservations = async () => {
        instance.get('reservations/restaurant')
            .then((res) => {
                setData(res.data)
            })
    ...
    if (typeof data === "object" && data.length > 0) {
        return (
            <Accordion className="mt-2" variant="splitted" selectionBehavior="toggle" defaultExpandedKeys={["0"]}>
                {data.map((item, index) => (
                    <AccordionItem key={index} aria-label={item.restaurantName} className="relative" title={<>{translations.ReservationNumber}: {item.reservationNumber}</>}>
                        <Divider className="-mt-2 mb-2" />
                        <a className="text-gradient-zielony" href={"/api/reservations/restaurant/pdf?reservationNum=" + item.reservationNumber} target="_blank">{translations.seeReservation}</a>
                        <p className="dark:text-white text-black"><strong>{translations.restaurantName}:</strong> <NavLink className="text-gradient-zielony text-base" to={"/restaurants/" + item.restaurantName}>{item.restaurantName}</NavLink></p>
                        <p className="dark:text-white text-black"><strong>{translations.date}:</strong> {item.date}</p>
